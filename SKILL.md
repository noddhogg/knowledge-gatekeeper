...
description: 个人知识库的治理层 — 把收藏了却没看的东西，变成真的会看的东西。四档质量门禁、成页门禁（防注水）、单一账本增量、断点续跑、防丢闭环、页面结构规范、平台接入清单（集成获取，不做实现）。触发词：知识库治理 / 质量门禁 / 成页门禁 / 注水检测 / 素材定档 / 收藏精炼 / 知识库防丢 / 账本续跑 / 笔记入库审查 / 平台接入
---
...
但**素材过关 ≠ 页面过关**。成页在层 ④（呈现），这一环原本一条判据都没有。

一次真实事故说明了缺口有多致命：某 C++ 学习知识库 41 页，素材定档全部合规，
成页由 AI 批量生成，要求「每页补到 ≥2000 字」。结果——

| 指标 | 实测 |
|---|---|
| 页面总字数 | 1341 – 1874（极差 **533**，被配额抹平） |
| 真实知识量 | 0 – 1734（极差 **1734**） |
| 含 AI 指令泄漏的标题 | 41 / 41 页，共 **237 处** |
| 重复样板占比 | **80%** |

页面上真的印着「【AI 扩写样例】…（补到 ≥2000 字 + 实操）」「再补」「三补」…「终极密集」。
**发给 AI 的工单，被贴在了成品上。**

最致命的是**它骗过了所有既有检查**：账本状态全 `done`、`page` 路径真实存在、
...
字数配额奖励的是「看起来一样长」，不是「有东西」。上表就是明证：
总字数极差只有 533，真实知识量极差 1734——**配额把差异抹平了，不是消除了**。
...
上例 41 页的真实知识合计只有 **1.3 万字**，平均到每页 323 字——按 800 字门槛只够 **16 页**。
